...
class EjercicioService:
    ...
    def get_por_nombre_patrones(patrones: List[str]) -> List[Ejercicio]:
        ...
        """
        if patrones is None:
            return Ejercicio.query.order_by(Ejercicio.id_ejercicio).all()

        _patrones = PatronMovimiento.query.filter(
            PatronMovimiento.nombre.in_(patrones)
        ).all()

        if _patrones is None:
            return _patrones

        return Ejercicio.query.filter(
            Ejercicio.id_patron.in_([p.id_patron_movimiento for p in _patrones])
        ).all()
    ...
    def create_ejercicio(nombre: str, patron: str) -> Ejercicio:
        ...
        nuevoEjercicio = Ejercicio(nombre=nombre, patron=_patron)
